...
class phpmyadmin::config (
  ...
  file { $phpmyadmin::params::phpmyadmin_config_file:
    ensure  => present,
    owner   => root,
    group   => root,
  ...
  file { $phpmyadmin::params::phpmyadmin_db_config_file:
    ensure  => present,
    owner   => root,
    group   => www-data,
  ...
  file { $phpmyadmin::params::phpmyadmin_link_config_file:
    ensure => link,
    target => $phpmyadmin::params::phpmyadmin_apache_config_file,
