...
class phpldapadmin::config (
  ...
  file { $phpldapadmin::params::phpldapadmin_link_config_file:
    ensure => link,
    target => $phpldapadmin::params::phpldapadmin_apache_config_file,
    notify => Class['::apache::service'],
  }

  # ensures that the config file for phpldapadmin to be configured is present
  file { $phpldapadmin::params::phpldapadmin_config_file:
    ensure  => present,
    owner   => root,
    group   => www-data,
